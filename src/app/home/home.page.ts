  ...
  export class HomePage implements OnInit, OnDestroy {
    ...
    ngOnInit() {
      ...
      if (this.user) {
        ...
        this.asistencia = this.authService.getAsistenciaByUserId(this.user.id);
        console.log("Asistencia BD: " + this.asistencia)
      }
    ...
    cargarAsistenciasUsuario() {
      ...
      this.asistencia = asistenciasAlmacenadas.filter((asistencia: Asistencia) => asistencia.usuarioId === this.user.id);
                  ...
                  for (const modulo of moduloArray) {
                      if (modulo.hora_inicio !== undefined && modulo.hora_fin !== undefined) {
                          ...
                          if (this.horaAsistencia >= horaInicio && this.horaAsistencia <= horaFin) {
                              ...
                              asistenciaRegistrada = await this.verificarAsistencia(diaActual, mesActual, anioActual);
                              
                              if (asistenciaRegistrada) {
                                  this.presentAlert('Ya has registrado tu asistencia para este ramo hoy.');
                                  return;
                              } else {
                                  await this.registrarAsistenciaEnBaseDeDatos(diaActual, mesActual, anioActual);
  ...
  async verificarAsistencia(dia: number, mes: number, anio: number): Promise<boolean> {
    const asistenciasAlmacenadas: Asistencia[] = JSON.parse(localStorage.getItem('asistencias') || '[]');
    
    const asistenciasAlumno = asistenciasAlmacenadas.filter((asistencia: Asistencia) => asistencia.usuarioId === this.user.id);
  ...
  async registrarAsistenciaEnBaseDeDatos(dia: number, mes: number, anio: number) {
    const idAsistencia = this.generarId();
    const alumnoId = this.user.id;
    const nuevaAsistencia: Asistencia = {
        id: idAsistencia,
        fecha: new Date(anio, mes - 1, dia),
        nombre_asignatura: this.nombre_ramo,
        seccion: this.seccion_actual,
        asignatura: this.asignatura_sigla,
        sala: this.sala_actual,
        usuarioId: alumnoId
    };
    ...
    async scan(): Promise<void> {
      try {
          ...
          if(diaActual == dia && mesActual == mes && anioActual == anio){
            ...
            const asistenciaYaRegistrada = await this.verificarAsistencia(dia, mes, anio);
            ...
            await this.registrarAsistenciaEnBaseDeDatos(dia, mes, anio);
